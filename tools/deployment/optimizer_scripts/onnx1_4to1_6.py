...
def replace_all_attribute_to_const_node_in_pad_node(g):
    ...
    for node in g.node:
        ...
        pad_mode = 'constant'
        pad_value_node = helper.list_to_constant(node.name + '_pad_value', [],
                                                 [0.0])  # need scalar
        for att in node.attribute:
            if att.name == 'mode':
                pad_mode = helper.get_var_attribute_by_name(
                    node, 'mode', 'string')
            if att.name == 'pads':
                pad_loc_node = helper.list_to_constant(node.name + '_pad_loc',
                                                       [len(att.ints)],
                                                       att.ints)
            if att.name == 'value':
    ...
    for node in g.node:
        ...
        if scale_value_node.op_type != "Constant":
            raise TypeError(
                ...
                + node.name + ' , you might need to do constant folding first')

        roi_node = helper.list_to_constant(node.name + '_roi_value', [0], [])

        new_node = onnx.helper.make_node(
            "Resize", [node.input[0], roi_node.name, scale_value_node.name],
            [node.output[0]],
            name=node.output[0],
