...
def fuse_consecutive_transposes(g):
    ...
    for node in g.node:
        ...
        new_trans_node = onnx.helper.make_node(
            'Transpose', [pre_node.input[0]], [node.output[0]],
            name=node.name,
...
def fuse_consecutive_reducemean(g):
    ...
    for node in g.node:
        ...
        new_gap_node = onnx.helper.make_node(
            'GlobalAveragePool', [pre_node.input[0]],
...
def fuse_slice_nodes_into_conv(g):
    ...
    def remove_nodes(input_name):
        ...
        if len(following_nodes
               ) == 1 and following_nodes[0].op_type == 'Concat':
    ...
    def remove_value_infos(input_name):
        ...
        if following_nodes[0].op_type == 'Concat':
            return
        for following_node in following_nodes:
            output_value = helper.find_value_by_name(g,
                                                     following_node.output[0])
    ...
    for input_value in g.input:
        ...
        # Construct weight
        weight_np = np.zeros((input_shape[1] * 4, input_shape[1], 3, 3),
                             dtype=np.float32)
        for i in range(4):
            # Check each branch
            slice_position = get_slice_position(concat_node.input[i])
            for j in range(channel_num):
                weight_np[i * channel_num + j, j, slice_position[0],
                          slice_position[1]] = 1
        weight_node = helper.numpy_to_constant(concat_node.name + '_weight',
                                               weight_np)
        # Construct Conv node
        new_conv = onnx.helper.make_node(
            'Conv', [input_value.name, concat_node.name + '_weight'],
