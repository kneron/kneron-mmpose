...
def setup_current_opset_version(m):
    ...
    if __ONNX_VERSION__ not in [11]:
        raise RuntimeError('Only support opset 11, but got ' +
                           str(__ONNX_VERSION__))
...
def concatenate(data_sets, axis):
    ...
    for i in range(split_num):
        for j in range(len(shapes)):
            ...
            total_flat_data.extend(flat_data_sets[j][i * block_size:(i + 1) *
                                                     block_size])
...
def broadcast_data_sets(data_set_1, data_set_2):
    ...
    list_a, list_b = (shape1,
                      shape2) if len(shape1) > len(shape2) else (shape2,
                                                                 shape1)
