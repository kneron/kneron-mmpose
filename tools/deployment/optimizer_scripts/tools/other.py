...
def remove_nodes(g, cut_nodes=[], cut_types=[]):
    ...
    for node in node_to_delete:
        ...
        for input_name in node.input:
            ...
            if value is not None and helper.find_output_by_name(
                    g, input_name) is None and value.name not in new_output:
    ...
    while output_values:
        ...
        if output_value.name in reachable_values:
            ...
            real_outputs = [
                i for i in output_mapping[output_value.name]
                if i.name in reachable_values
            ]
            logging.info("Replace output {} with {}".format(
...
def topological_sort(g):
    ...
    if in_degree:
        raise RuntimeError("Unreachable nodes exist: {}".format(
            in_degree.keys()))
    if node_map:
        ...
        if auto_pad is not None and auto_pad != 'NOTSET':
            if auto_pad == 'SAME_LOWER' or auto_pad == 'SAME_UPPER':
                new_output_value_info = onnx.helper.make_tensor_value_info(
                    node.output[0],
                    input_value_info.type.tensor_type.elem_type, input_shape)
...
def add_nop_conv_after(g, value_names):
    ...
    for value_name in value_names:
        ...
        conv_node = onnx.helper.make_node(
            "Conv", [value_name, weight_node.output[0]], [node_name],
            name=node_name,
            dilations=[1, 1],
            group=channel,
            kernel_shape=[1, 1],
...
def add_shift_scale_bn_after(g, value_name, channel_shift, channel_scale):
    ...
    scale_node = helper.list_to_constant(node_name + "_scale",
                                         [len(channel_scale)], channel_scale)
    bias_node = helper.list_to_constant(node_name + "_bias",
                                        [len(channel_shift)], channel_shift)
...
def deconv_to_conv_info_extraction(input_size, node_proto):
    ...
    attr["group"] = helper.get_var_attribute_by_name(node_proto, "group",
                                                     "int")
    ...
    if attr["output_padding"] is None:
        if attr["auto_pad"] == "SAME_LOWER" or attr["auto_pad"] == "SAME_UPPER":
            attr["output_padding"] = [
                attr["strides"][0] - 1, attr["strides"][1]
            ]
        else:
            attr["output_padding"] = [
    ...
    if attr["auto_pad"] == "SAME_LOWER" or attr["auto_pad"] == "SAME_UPPER":
        pad1_h = attr["kernel_shape"][0] - (attr["kernel_shape"][0] -
                                            1) // 2 - 1
        pad1_w = attr["kernel_shape"][1] - (attr["kernel_shape"][1] -
                                            1) // 2 - 1
        head_h = min(attr["kernel_shape"][0] // 2,
                     (attr["output_padding"][0] + 1) // 2)
        head_w = min(attr["kernel_shape"][1] // 2,
                     (attr["output_padding"][1] + 1) // 2)
        tail_h = attr["output_padding"][0] - head_h
        tail_w = attr["output_padding"][1] - head_w
        attr["conv_pads"] = [
            pad1_h + head_h, pad1_w + head_w, pad1_h + tail_h, pad1_w + tail_w
        ]
        ...
        if sum_of_pads == 0:
            ...
            tail_w = attr["output_padding"][1] - head_w
            attr["conv_pads"] = [
                pad1_h + head_h, pad1_w + head_w, pad1_h + tail_h,
                pad1_w + tail_w
            ]
        else:
            # Calculate output shape
            tmp_output_shape = [0, 0]
            tmp_output_shape[0] = attr["strides"][0] * (
                input_size[2] - 1) + attr["output_padding"][0] + attr[
                    "kernel_shape"][0] - attr["pads"][0] - attr["pads"][2]
            tmp_output_shape[1] = attr["strides"][1] * (
                input_size[3] - 1) + attr["output_padding"][1] + attr[
                    "kernel_shape"][1] - attr["pads"][1] - attr["pads"][3]
            ...
            # Calculate padding
            total_padding = [0, 0]
            total_padding[0] = tmp_output_shape[0] - tmp_center_shape[
                0] + attr["kernel_shape"][0] - 1
            total_padding[1] = tmp_output_shape[1] - tmp_center_shape[
                1] + attr["kernel_shape"][1] - 1
            if total_padding[0] < 0 or total_padding[1] < 0:
                raise RuntimeError(node_proto.name +
                                   " cannot infer conv padding.")
            conv_pads_ = [0] * 4
...
def split_ConvTranspose(model):
    ...
    for node in g.node:
        ...
        upsample_output_shape[2] = (input_shape[2] -
                                    1) * attr["strides"][0] + 1
        upsample_output_shape[3] = (input_shape[3] -
                                    1) * attr["strides"][1] + 1
        ...
        upsample_node = onnx.helper.make_node(
            "Upsample", [node.input[0], upsample_scale_name],
            [upsample_node_name],
            name=upsample_node_name,
            mode="zeros")
        upsample_value_info = onnx.helper.make_tensor_value_info(
            upsample_node_name, input_value.type.tensor_type.elem_type,
        ...
        if attr["group"] != input_shape[1]:
            ...
            new_weight_node = helper.numpy_to_constant(node.input[1],
                                                       new_weight_np)
