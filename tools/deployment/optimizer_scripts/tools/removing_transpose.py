...
def swap_transpose_with_single_next_node(g):
    swapped = False
    passable_nodes = set([
        'Relu', 'Neg', 'LeakyRelu', 'Sqrt', 'Reciprocal', 'Add', 'Mul', 'Tanh'
    ])
    for node in g.node:
        trans_node = node
        ...
        for input_node in input_nodes:
            if input_node.name == trans_node.name:
                ...
                for out_node in output_nodes:
                    modhelper.replace_node_input(out_node, next_node.output[0],
                                                 trans_node.name)
    ...
    for node in g.node:
        ...
        for i in range(len(output_nodes)):
            ...
            new_trans_node = onnx.helper.make_node(
                'Transpose', [node.output[0]], [new_trans_node_name],
                name=new_trans_node_name,
                perm=permutation)
            modhelper.replace_node_input(output_nodes[i], node.output[0],
                                         new_trans_node_name)
...
def annihilate_transposes(g):
    ...
    for node in g.node:
        ...
        for out_node in out_nodes:
            modhelper.replace_node_input(out_node, node.output[0],
                                         pre_node.input[0])
...
def split_transpose_for_multiple_next_nodes(g):
    ...
    for node in g.node:
        ...
        for i in range(len(output_nodes)):
            ...
            new_trans_node = onnx.helper.make_node(
                'Transpose', [node.input[0]], [new_trans_node_name],
                name=new_trans_node_name,
                perm=list(node.attribute[0].ints))
            modhelper.replace_node_input(output_node, node.output[0],
                                         new_trans_node.output[0])
            g.node.extend([new_trans_node])
