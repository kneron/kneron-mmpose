...
def replace_Squeeze_with_Reshape(g):
    ...
    for node in g.node:
        ...
        new_node = onnx.helper.make_node(
            "Reshape", [node.input[0], node.name + "_shape"],
            node.output,
            name=node.name)
...
def replace_depthwise_1x1_with_bn(g):
    ...
    for node in g.node:
        ...
        if "group" not in attr_map or attr_map["group"].i == 1:
            continue
        if attr_map["kernel_shape"].ints[0] != 1 or attr_map[
                "kernel_shape"].ints[1] != 1:
            continue
        ...
        if len(node.input) == 3:
            ...
            bias_node = helper.list_to_constant(bias_name,
                                                [attr_map["group"].i],
                                                [0.0] * attr_map["group"].i)
            g.node.extend([bias_node])
        # Construct mean and vars
        mean_name = node.name + "_mean"
        mean_node = helper.list_to_constant(mean_name, [attr_map["group"].i],
                                            [0.0] * attr_map["group"].i)
        var_name = node.name + "_var"
        var_node = helper.list_to_constant(var_name, [attr_map["group"].i],
                                           [1.0] * attr_map["group"].i)
        g.node.extend([mean_node, var_node])
        # Convert
        bn_node = onnx.helper.make_node(
            op_type='BatchNormalization',
            inputs=[
                node.input[0], node.input[1], bias_name, mean_name, var_name
            ],
            outputs=node.output,
            name=node.name,
            epsilon=0.00001,
...
def replace_sub_to_bn(g):
    ...
    for node in g.node:
        ...
        bn_node = onnx.helper.make_node(
            'BatchNormalization', [
                real_input_name, scale_value_node.output[0],
                new_add_value_node.output[0], mean_value_node.output[0],
                variance_value_node.output[0]
