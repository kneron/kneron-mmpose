    ...
    while keep_folding:
        ...
        for node in g.node:
            ...
            if constant_folding_nodes[node.op_type](g, node):
                ...
                logging.debug("Constant nodes and %s %s are skipped.",
                              node.op_type, node.name)
    return folded
...
def duplicate_constant_node(g):
    ...
    for node in g.node:
        ...
        foldable_output_nodes = list(
            filter(lambda n: n.op_type in constant_folding_nodes.keys(),
                   output_nodes))
        ...
        for i in range(len(foldable_output_nodes)):
            logging.debug(
                "Found constant %s and %s %s are availble for folding. Duplicate constant.",
                node.name, foldable_output_nodes[i].op_type,
                foldable_output_nodes[i].name)
...
def slice_constant_folding_Opset_11(g, node):
    ...
    axes_node = None if len(
        node.input) <= 3 else helper.find_node_by_output_name(
            g, node.input[3])
    if not axes_node:
    ...
    steps_node = None if len(
        node.input) <= 4 else helper.find_node_by_output_name(
            g, node.input[4])
    if not steps_node:
    ...
    new_node = helper.list_to_constant(node.output[0],
...
def bn_constant_folding(g, node):
    ...
    # Calculate new node
    new_data = scale_data * (
        input_data - mean_data) / np.sqrt(var_data + epsilon) + bias_data
