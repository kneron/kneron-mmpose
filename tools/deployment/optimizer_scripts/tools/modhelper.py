...
def delete_nodes(g, node_list):
    ...
    for node in node_to_delete:
        ...
        if len(node.input) == 0:
            print(
                "Deleting an Constant node. Please make sure you also delete all its following nodes"
            )
        elif len(node.input) > 1:
            print(
                "Warning: Node {} has more than one input. This script cannot delete merge nodes."
                .format(node.name))
        ...
        elif len(following_nodes) > 0 and len(
                node.input) == 1 and helper.find_input_by_name(
                    g, node.input[0]) is not None:
