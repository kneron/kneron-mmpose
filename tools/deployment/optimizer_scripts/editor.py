...
parser.add_argument(
    '--pixel-shift-value',
    dest='pixel_shift_value',
    type=str,
    nargs='+',
    help=
    '(per channel) set pixel value shift bn layer at model front for normalization( e.g. --pixel_shift_value "[104.0, 117.0, 123.0]" )'
)
parser.add_argument(
    '--pixel-scale-value',
    ...
    nargs='+',
    help=
    '(per channel) set pixel value scale bn layer at model front for normalization( e.g. --pixel_scale_value "[0.0078125, 0.0078125, 0.0078125]" )'
)
...
if args.pixel_shift_value is not None or args.pixel_scale_value is not None:
    ...
    if args.pixel_shift_value is not None and len(args.pixel_shift_value) == 1:
        pixel_shift_value = [
            float(n)
            for n in args.pixel_shift_value[0].replace('[', '').replace(
                ']', '').split(',')
        ]
    ...
    if args.pixel_scale_value is not None and len(args.pixel_scale_value) == 1:
        pixel_scale_value = [
            float(n)
            for n in args.pixel_scale_value[0].replace('[', '').replace(
                ']', '').split(',')
        ]
    ...
    for i_n in g.input:
        if i_n.type.tensor_type.shape.dim[1].dim_value != 3:
            raise ValueError(
                "Only support 3 channel input, found input node channel not equal to 3: node name... "
                + i_n.name)
        other.add_shift_scale_bn_after(g, i_n.name, pixel_shift_value,
                                       pixel_scale_value)
...
if args.rename_output:
    if len(args.rename_output) % 2 != 0:
        ...
        for i in range(0, len(args.rename_output), 2):
            other.rename_output_name(g, args.rename_output[i],
                                     args.rename_output[i + 1])
