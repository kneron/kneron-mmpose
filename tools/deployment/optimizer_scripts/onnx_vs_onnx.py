...
def onnx_model_results(path_a, path_b, total_times=10):
    ...
    assert len(outputs_a) == len(
        outputs_b), 'Two models have different output numbers.'
    for i in range(len(outputs_a)):
        out_shape_a, out_shape_b = outputs_a[i].shape, outputs_b[i].shape
        out_shape_a = list(
            map(lambda x: x if type(x) == type(1) else 1, out_shape_a))
        out_shape_b = list(
            map(lambda x: x if type(x) == type(1) else 1, out_shape_b))
        assert out_shape_a == out_shape_b, 'Output {} has unmatched shapes'.format(
            i)
    ...
    input_names_a, input_names_b = set([ele.name for ele in inputs_a
                                        ]), set([ele.name for ele in inputs_b])
    init_names_a, init_names_b = set([ele.name for ele in init_a
                                      ]), set([ele.name for ele in init_b])
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(
        description=
        "Compare two ONNX models to check if they have the same output.")
