...
def compare_tflite_and_onnx(tflite_file, onnx_file, total_times=10):
    ...
    # Compare input shape
    assert (len(onnx_input_shape) == len(tflite_input_shape)
            ), "TFLite and ONNX shape unmatch."
    assert (onnx_input_shape == [
        tflite_input_shape[0], tflite_input_shape[3], tflite_input_shape[1],
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(
        description=
        "Compare a TFLite model and an ONNX model to check if they have the same output."
    )
